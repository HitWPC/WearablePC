...
/*
    ...
    private void judge(BluetoothGattCharacteristic characteristic) {
        String data = new String(characteristic.getValue());
        if (data != null) {
//                    Log.d(TAG, data);
            tempSB.append(data);
            if (ifHasDataNeeded(tempSB)) {
                storeBDdata(getFormatLatlng(tempSB));
                tempSB.delete(0, tempSB.length());
            } else ;
        }
    }

    private void storeBDdata(LatLng latLng){
        BDTable bddata = new BDTable();
        bddata.setLatitude(latLng.latitude);
        bddata.setLongitude(latLng.longitude);
        boolean success = bddata.save();
        if (success){
            Log.d(TAG, "BD保存成功"+latLng.longitude+"  "+latLng.latitude);
        }else {
            Log.d(TAG, "BD保存失败");
        }
    }

    private boolean ifHasDataNeeded(StringBuilder sb){
        Pattern p;
        if(sb != null){
            p = Pattern.compile("\\$GNRMC,.+,A,.+,[N|S],.+,[W|E],.*");
    ...
    private LatLng getFormatLatlng(StringBuilder sb){
        LatLng latLng;
