 ...
 * 固定采集频率50Hz
 */
public class ActionAdaptiveService extends Service{
    ...
    public void onDestroy() {
        Log.d(ActionAdaptiveService.class.getSimpleName(), "onDestroy");
    ...
    public void recg(ActionData actionData){
        ...
        if(count >= peakCount){
            ...
            SVM.exec.execute(new Runnable() {
                @Override
                public void run() {
                    FileOperateUtil.saveMemoryAndTime(lastIdBackup, RecognitionType.ALGORITHM_ADAPTIVE);
                }
            });
            count = 0;
        }else {
            if (accX.size() == 2 * BEST_FREQ) {
                Log.d(ActionAdaptiveService.class.getSimpleName(), "startTime = " + timeList.get(0) + ",endTime = " + timeList.get(timeList.size() - 1));
                lastIndex = FeatureCore.calculateFeature(true, RecognitionType.ALGORITHM_ADAPTIVE,
                        Preprocess.medFilt(accX), Preprocess.medFilt(accY), Preprocess.medFilt(accZ), timeList.get(0), timeList.get(timeList.size() - 1));

                clearAllList();
                count++;
                Log.d(ActionAdaptiveService.class.getSimpleName(), "count = " + count);
            } else {
